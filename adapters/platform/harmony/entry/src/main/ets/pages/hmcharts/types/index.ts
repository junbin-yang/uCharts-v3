...
export interface ChartOptions {
  type: ChartType
  context: AnyType          //CanvasRenderingContext2D
  categories?: string[]     //图表数据集，部分图表类型不需要categories
  series?: Series[]          //图表数据集，请按不同图表类型传入对应的标准数据
  ...
  color?: string[]            //主题颜色，16进制颜色格式
  padding?: number[]         //画布填充边距，顺序为上右下左，例如[10,15,25,15]
  ...
  enableMarkLine?: boolean   //是否启用标记线功能，也可做为隐藏图表区域内的标记线的开关，默认false
  scrollPosition?: 'current'|'left'|'right' //连续更新数据时，滚动条的位置。默认'current'。可选值："current"当前位置,"left"左对齐,"right"右对齐
  yAxis?: YAxisOptions       //Y轴选项
  xAxis?: XAxisOptions       //X轴选项
  legend?: LegendOptions     //图例配置
  title?: TitleOptions       //标题配置，此配置仅适用于ring、arcbar、gauge，直角坐标系图表可在X轴配置/Y轴配置中设置标题
  subtitle?: TitleOptions    //副标题配置，此配置仅适用于ring、arcbar、gauge，直角坐标系图表可在X轴配置/Y轴配置中设置标题
  extra?: Extra              //扩展配置
...
/**
 * 图表类型
 * column 柱状图 | mount 山峰图 | bar 条状图 | line 折线图 | area 区域图 | scatter 散点图 | bubble 气泡图 | mix 混合图 | pie 饼状图 | ring 圆环图 | rose 玫瑰图 | radar 雷达图 | arcbar 进度条 | gauge 仪表盘 | funnel 漏斗图 | candle K线图 | map 地图
 */
export type ChartType = "column" | "mount" | "bar" | "line" | "area" | "scatter" | "bubble" | "mix" | "pie" | "ring" | "rose" | "radar" | "arcbar" | "gauge" | "funnel" | "candle" | "map"
...
export interface YAxisOptionsData {
  ...
  //formatter?: (value: YAxisOptionsData,index: number,opts: ChartOptions) => string  //格式化Y轴文案显示
...
export interface XAxisOptions {
  ...
  titleOffsetX?: number     //标题横向偏移距离，负数为向左偏移，正数向右偏移
  titleFontColor?: string   //标题字体颜色，默认#666666
  //formatter?: (value,index,opts) => string
