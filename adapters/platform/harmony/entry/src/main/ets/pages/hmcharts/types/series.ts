...
interface BaseSeries {
  ...
  //formatter?: (value,index,series,opts) => string
}

// 柱状图数据系列
export interface ColumnSeriesCustomData {   //主要作用为柱状图自定义颜色
  value: number
  color: string
}
export interface ColumnSeries extends BaseSeries {
  data: Array<number|ColumnSeriesCustomData>  //数据值，如果传入null图表该处出现断点
}

// 山峰图数据系列
export interface NameAndValueData {
  name: string,
  value: number
}
...
export interface MixedSeries extends BaseSeries {
  ...
  data: Array<number|ColumnSeriesCustomData|[number,number]>       //数据值
...
export interface CandleSeries extends BaseSeries {
  data: Array<number[]>  //数据值
  seriesMA?: Array<number[]>  //K线图自定义折线数据列表，内容同series，仅在opts.extra.candle.average.show为false时调用此数据
}

// 地图数据系列
export interface MapSeries extends BaseSeries {
  //data: Array<number[]>  //数据值
}
